...
if ! command -v srectl &> /dev/null; then
    echo -e "${RED}srectl not found.${NC}"
    echo "srectl (SRE Agent CLI) is required for automated setup."
    echo "Until srectl is publicly available, follow the manual setup:"
    echo "  → docs/SRE-AGENT-CONFIG.md"
    echo "  → docs/SRE-AGENT-SETUP.md"
    exit 1
fi
